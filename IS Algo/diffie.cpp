...
class DiffieHellman
{
    public:
        ...
        DiffieHellman(long long int p1,long long int g1,long long int x1,long long int y1)
        {
            ...
            x = x1;
            y = y1;
            a=power(g,x,p);  // generates key for alice g raised to power x mod p
            b=power(g,y,p);  // generates key for bob g raised to power y mod p
            A = power(b,x,p);  // exchange of generated key 
            B = power(a,y,p);
        }
